...
public:
    ...
    ThreeSieves(size_t k, MHR &f, double m, double epsilon, THRESHOLD_STRATEGY strategy, size_t T, vector<UtilityFunction> &FC) : SubsetSelectionAlgorithm(k, f), threshold(k * m), epsilon(epsilon), strategy(strategy), T(T), t(0), FC(FC)
    {
        assert(T >= 1);
    }
