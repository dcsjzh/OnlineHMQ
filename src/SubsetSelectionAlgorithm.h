...
public:
    ...
    double fval_sieves;
    vector<size_t> idxs;
    // unsigned long maxn; // maximum num of elements stored

    SubsetSelectionAlgorithm(size_t k, MHR &f) : k(k), f(f)
    {
        is_fitted = false;
        fval = 0;
        fval_sieves = 0;
    ...
    virtual void fit(vector<Point> const &DataSet, unsigned int iterations = 1)
    {
        for (unsigned int i = 1; i <= iterations; ++i)
        {
            for (auto &cur_point : DataSet)
            {
                ...
                // if (this->get_num_elements_stored() > maxn)
                // {
                //     maxn = this->get_num_elements_stored();
                // }
            }
        }
    }
    ...
    {
        if (!is_streaming) // just for the Greedy algorithm
        {
