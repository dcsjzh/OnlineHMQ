...
private:
    ...
    public:
        ...
        Sieve(size_t k, MHR &f, double threshold) : SubsetSelectionAlgorithm(k, f), threshold(threshold) 
        {
...
public:
    ...
    SieveOnline(size_t k, MHR &f, double epsilon, vector<UtilityFunction> &FC) : SubsetSelectionAlgorithm(k, f), FC(FC) , epsilon(epsilon)
    {
        ...
        else
        {
            ...
            for (size_t l = 0; l < tmp_solution.size(); ++l) 
            {
                ...
                for (size_t j = 0; j < happinesses.size(); ++j)
                {
                    ...
                    happy_ratio = min(happy_max / FC[j].fmax, f.get_tau());
                    sum += happy_ratio;
                }
